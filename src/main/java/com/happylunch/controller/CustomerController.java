...
import com.happylunch.entity.OrderedProduct;
import com.happylunch.entity.Product;
import com.happylunch.repository.CustomerOrderRepository;
...
public class CustomerController {
    ...
    private void populateProduct(CustomerOrder customerOrder) {
        ...
        for (String item: data) {
            array = item.split(":");
            if (array.length == 2) {
                product = productRepository.findById(new Integer(array[0]) + 1);
            }
            if (null != product) {
                orderedProduct = new OrderedProduct();
                orderedProduct.setQuantity(Short.parseShort(array[1]));
                sum = product.getPrice().multiply(new BigDecimal(orderedProduct.getQuantity()));
